...
module.exports.getList = async (
  ...
  LEFT JOIN category ON category.articles_idx = articles.articles_idx`;
  let values = [];
  if (options) {
...
module.exports.detailgetList = async (connection, options) => {
  let query = `SELECT * FROM articles
  LEFT JOIN category ON category.articles_idx = articles.articles_idx`;
  let values = [];
  if (options) {
...
module.exports.update = async (connection, options) => {
  ...
  // UPDATE articles SET ? WHERE user_idx = ?
  return await db.query(connection, {
    query: query,
    values: values,
  });
